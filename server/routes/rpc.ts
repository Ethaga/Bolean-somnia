...
export const handleRpcProxy: RequestHandler = async (req, res) => {
  ...
  if (req.method === "GET") {
    return res.json({ ok: true, rpc: process.env.SOMNIA_RPC_URL || DEFAULT_RPCS[0] });
  }

  const payload = req.body;
  if (!payload) return res.status(400).json({ error: "Missing JSON-RPC body" });

  const rpcs = [process.env.SOMNIA_RPC_URL, ...DEFAULT_RPCS].filter(Boolean) as string[];
  ...
  return res.status(502).json({ error: "Bad Gateway", message: "All upstream RPCs failed", details: errors });
};
